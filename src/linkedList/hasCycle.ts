...
export function hasCycle(head: ListNode | null): boolean {
  ...
  return run(head, runner.next)
}

function run(current: ListNode, runner: ListNode | null): boolean {
  if (runner === null || runner.next === null) return false

  if (runner.next === current.next) return true

  const nextRunner = runner.next?.next

  if (!nextRunner) return false
  if (!current.next) return false

  return run(current.next, nextRunner)
}
